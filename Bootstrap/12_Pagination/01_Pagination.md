...
To create a basic pagination, add the `.pagination` class to an `<ul>` element. Then add the `.page-item` to each `<li>` element and a `.page-link` class to each `<a>` inside `<li>`:
```html
<ul class="pagination">
...
## `.justify-content-center` Pagination Alignment
```html
